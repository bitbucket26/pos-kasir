                    ...
                    <div class="card shadow mb-4">
                        <?php
                            ...
                            if (empty($a)) {
                                ...
                                $query0 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day0' group by date(tanggalbayar)");
                                $row0 = $query0->fetch_array();
                                $total0[] = $row0['total'];
                                // echo json_encode($total);

                                $query1 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day1' group by date(tanggalbayar)");
                                $row1 = $query1->fetch_array();
                                $total1[] = $row1['total'];

                                $query2 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day2' group by date(tanggalbayar)");
                                $row2 = $query2->fetch_array();
                                $total2[] = $row2['total'];

                                $query3 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day3' group by date(tanggalbayar)");
                                $row3 = $query3->fetch_array();
                                $total3[] = $row3['total'];

                                $query4 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day4' group by date(tanggalbayar)");
                                $row4 = $query4->fetch_array();
                                $total4[] = $row4['total'];

                                $query5 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day5' group by date(tanggalbayar)");
                                $row5 = $query5->fetch_array();
                                $total5[] = $row5['total'];

                                $query6 = mysqli_query($koneksi,"SELECT sum(total) as total from kasir5 where (tanggalbayar)= '$day6' group by date(tanggalbayar)");
