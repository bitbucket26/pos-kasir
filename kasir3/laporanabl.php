                ...
                <div class="col-xl-6" >
                    ...
                    if (mysqli_connect_error()){
                        ...
                        if (empty($a)) {
                            ...
                            $query = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day' group by date(tanggal)");
                            $row = $query->fetch_array();
                            $totals[] = $row['total'];
                         
                            $query1 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day1' group by date(tanggal)");
                            $row1 = $query1->fetch_array();
                            $total1[] = $row1['total'];
                         
                            $query2 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day2' group by date(tanggal)");
                            $row2 = $query2->fetch_array();
                            $total2[] = $row2['total'];
                           
                            $query3 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day3' group by date(tanggal)");
                            $row3 = $query3->fetch_array();
                            $total3[] = $row3['total'];
                           
                            $query4 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day4' group by date(tanggal)");
                            $row4 = $query4->fetch_array();
                            $total4[] = $row4['total'];
                           
                            $query5 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day5' group by date(tanggal)");
                            $row5 = $query5->fetch_array();
                            $total5[] = $row5['total'];
                            
                            $query6 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day6' group by date(tanggal)");
                            $row6 = $query6->fetch_array();
                            $total6[] = $row6['total'];

                            $query7 = mysqli_query($koneksi,"SELECT sum(total) as total from ablkasir3 where (tanggal)= '$day6' group by date(tanggal)");
