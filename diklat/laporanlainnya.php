...
if (mysqli_connect_error()){
    ...
    {
        $query = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where YEAR(tanggalmulai)='$thn'");
...
<head>
    ...
    <title>Laporan Pendapatan Diklat Lainnya</title>
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                $query_sesi = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where (tanggalmulai)= '$day_sesi'");
            ...
            <div class="row">
                <div class="card border-left-danger shadow h-100 py-2 align-items-left" style="margin-left: 32px; margin-bottom: 20px; padding: 10px; width: 30%;">
                    <div class="card-body">
                        ...
                        <h6 class="card-text">Total Pendapatan Diklat Lainnya Hari Ini</h6>
                        ...
                        <div class="card shadow mb-4">
                            <?php
                                ...
                                if (empty($a)) {
                                    ...
                                    $query = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day' group by date(tanggalmulai)");
                                    $row = $query->fetch_array();
                                    $totals[] = $row['total'];
                                    // echo json_encode($total);

                                    $query1 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day1' group by date(tanggalmulai)");
                                    $row1 = $query1->fetch_array();
                                    $total1[] = $row1['total'];

                                    $query2 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day2' group by date(tanggalmulai)");
                                    $row2 = $query2->fetch_array();
                                    $total2[] = $row2['total'];

                                    $query3 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day3' group by date(tanggalmulai)");
                                    $row3 = $query3->fetch_array();
                                    $total3[] = $row3['total'];

                                    $query4 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day4' group by date(tanggalmulai)");
                                    $row4 = $query4->fetch_array();
                                    $total4[] = $row4['total'];

                                    $query5 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day5' group by date(tanggalmulai)");
                                    $row5 = $query5->fetch_array();
                                    $total5[] = $row5['total'];

                                    $query6 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day6' group by date(tanggalmulai)");
                                    $row6 = $query6->fetch_array();
                                    $total6[] = $row6['total'];

                                    $query7 = mysqli_query($koneksi,"SELECT sum(total) as total from diklatlainnya where DAY(tanggalmulai)= '$day6' group by date(tanggalmulai)");
                ...
                <div class="col-xl-6">
                    ...
                    <div class="card shadow mb-4">
                            <div class="card-header py-3">
                                <h5 class="m-0 font-weight-bold text-primary">Tabel Rekap Pendapatan Diklat Lainnya Per-Bulan</h5>
                            </div>
                            <div class="card-body">
                                <?php
                                    include "../koneksi.php";
                                    ...
                                    $result = mysqli_query($koneksi,"select year(tanggalmulai) as year, month(tanggalmulai) as month, sum(total) as total
                                                                    from diklatlainnya group by year(tanggalmulai), month(tanggalmulai)");
                                ?>
